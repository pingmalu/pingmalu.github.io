...
注意可以通过开启这个功能来用本机下载：remote.SSH.localServerDownload

比如commit id是：379934e04d2b3290cf7aefa14560f942e4212920

部署流程:

```sh
# 先下载：https://cursor.blob.core.windows.net/remote-releases/379934e04d2b3290cf7aefa14560f942e4212920/vscode-reh-linux-x64.tar.gz

# 去Linux创建目录
mkdir -p /root/.cursor-server/cli/servers/Stable-379934e04d2b3290cf7aefa14560f942e4212920/server

# 将下载的包解压
tar -zxf vscode-reh-linux-x64.tar.gz -C /root/.cursor-server/cli/servers/Stable-379934e04d2b3290cf7aefa14560f942e4212920/server --strip-components 1
# 注意：--strip-components 1 是必须的，为了确保解压后的内容直接在 ${commit_id} 文件夹下，而不是多出一层目录。
...
python静态检查插件：basedpyright

发布者：detachhead

关闭类型检查:

[antigravity://settings/basedpyright.analysis.typeCheckingMode](antigravity://settings/basedpyright.analysis.typeCheckingMode)

[trae://settings/basedpyright.analysis.typeCheckingMode](trae://settings/basedpyright.analysis.typeCheckingMode)

[codebuddy://settings/basedpyright.analysis.typeCheckingMode](codebuddy://settings/basedpyright.analysis.typeCheckingMode)


移动cursor活动栏到左侧，变成树状排列：

改成 "vertical"

[cursor://settings/activitybar.orientation](cursor://settings/activitybar.orientation)
...
在线看图分析 [http://cnij.imjoy.io/](http://cnij.imjoy.io/)
