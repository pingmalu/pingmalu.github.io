...
### Windows 商店的 Python 别名 干扰默认python启动处理方案

1. 禁用 Windows 商店的 Python 别名（最关键）

从你的 which python 输出可以看出，系统优先匹配到了 Windows 自动生成的 WindowsApps 目录下的 Python 脚本，这通常会干扰 pyenv。

- 按下 Win + Q 搜索 “管理应用执行别名” (Manage app execution aliases)。

- 找到 Python.exe 和 Python3.exe，将其开关全部 关闭。

2. 使用 pyenv 设置全局版本

在 Git Bash 中执行以下命令，告诉 pyenv 你想默认使用 3.11.6：
